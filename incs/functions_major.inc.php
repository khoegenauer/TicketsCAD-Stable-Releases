...
//		dump($row);
		...
		$totrack  = ((intval($row['mobile'])==0) || (($track_type != 8) && (empty($row['callsign']))))? "" : "&nbsp;&nbsp;<SPAN class = 'span_link' onClick = do_track('" .$callsign  . "');><U>Tracks</U>&nbsp;&nbsp;</SPAN>" ;
...
// tab 1
			...
			$tab_1 .= "<TR CLASS='odd'><TD COLSPAN=2 ALIGN='center'>" . $tofac . $todisp . $totrack . $to_log . $toedit . "&nbsp;&nbsp;<A HREF='{$_SESSION['unitsfile']}?func=responder&view=true&id=" . $row['unit_id'] . "'><U>View</U></A></TD></TR>";	// 08/8/02
			$tab_1 .= "</TABLE>";
